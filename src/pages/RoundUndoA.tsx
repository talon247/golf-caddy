...
export default function RoundUndoA() {
  ...
  // Touch handlers for swipe-to-undo
  ...
  function handleTouchMove(e: React.TouchEvent) {
    if (touchStartX.current === null || strokes === 0) return
    const deltaX = e.touches[0].clientX - touchStartX.current
    setSwipeArmed(deltaX < -50)
  }
  ...
  function handleMouseMove(e: React.MouseEvent) {
    if (mouseStartX.current === null || strokes === 0) return
    ...
    <main className="flex flex-col flex-1">
      ...
      {activeTab === 'round' ? (
        <div className="flex flex-col flex-1 p-4 gap-4">
          ...
          {/* Stroke counter — swipeable */}
          <div className="relative bg-[#faf7f2] rounded-2xl border border-[#e5e1d8] p-5 h-36 flex flex-col items-center justify-center gap-1 shadow-sm overflow-hidden">
            {/* Swipeable score row */}
            <div
              className="relative flex items-center justify-center gap-2 h-16 w-full select-none touch-none"
              style={{ touchAction: 'pan-y' }}
              aria-label="Swipe left to remove last shot"
            ...
            >
              ...
              <ChevronLeft
                size={20}
                className={`absolute left-4 top-1/2 -translate-y-1/2 text-[#2d5a27] transition-opacity duration-300 ${
                  strokes === 0 ? 'opacity-0' : swipeArmed ? 'opacity-80' : 'opacity-40'
                }`}
              />
              {/* Score number */}
              <span
                className={`text-6xl font-black leading-none flex items-center justify-center transition-all duration-150 ${
                  strokes > 0 ? 'text-[#2d5a27]' : 'text-gray-400'
                } ${swipeArmed ? 'translate-x-[-8px] opacity-60' : ''}`}
              >
                {strokes}
              </span>
            </div>
